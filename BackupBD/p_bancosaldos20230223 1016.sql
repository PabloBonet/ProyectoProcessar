...
BEGIN
    ...
    `c`.`cbancaria`,`b`.`banco`, `b`.`nombre`, `b`.`filial`, `b`.`cp`
    into @vcodcuenta, @vdetalle,@vcuentabco, @vidtipocta,@vidbanco,@vcbu, @vplcaja, @vcheques, @vcbancaria, @vbanco, @vnombre, @vfilial, @vcp
    from `cajabancos` `c`
    left join `bancos` `b` on`b`.`idbanco` = `c`.`idbanco`
    where `c`.`idcuenta`= pidcuenta ;
    ...
    select  sum(ifnull(`importe`,0))  into  @vingresos
