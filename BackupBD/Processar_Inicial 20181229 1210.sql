...
--
-- Create schema trsoftdbvacio
--

CREATE DATABASE IF NOT EXISTS trsoftdbvacio;
USE trsoftdbvacio;
...
INSERT INTO `monedash` (`moneda`,`nombre`,`cotizacion`,`fechacot`) VALUES 
...
DROP TABLE IF EXISTS `ctactefact`;
DROP VIEW IF EXISTS `ctactefact`;
CREATE ALGORITHM=UNDEFINED DEFINER=`tulior`@`%` SQL SECURITY DEFINER VIEW `ctactefact` AS select `f`.`entidad` AS `entidad`,sum((`f`.`total` * `t`.`opera`)) AS `saldo` from ((`trsoftdb`.`facturas` `f` left join `trsoftdb`.`comprobantes` `c` on((`f`.`idcomproba` = `c`.`idcomproba`))) left join `trsoftdb`.`tipocompro` `t` on((`c`.`idtipocompro` = `t`.`idtipocompro`))) where (`c`.`ctacte` = 'S') group by `f`.`entidad`;
...
DROP TABLE IF EXISTS `ctacterec`;
DROP VIEW IF EXISTS `ctacterec`;
CREATE ALGORITHM=UNDEFINED DEFINER=`tulior`@`%` SQL SECURITY DEFINER VIEW `ctacterec` AS select `r`.`entidad` AS `entidad`,sum((`r`.`importe` * `t`.`opera`)) AS `saldo` from ((`trsoftdb`.`recibos` `r` left join `trsoftdb`.`comprobantes` `c` on((`r`.`idcomproba` = `c`.`idcomproba`))) left join `trsoftdb`.`tipocompro` `t` on((`c`.`idtipocompro` = `t`.`idtipocompro`))) where (`c`.`ctacte` = 'S') group by `r`.`entidad`;
...
DROP TABLE IF EXISTS `ctactesaldo`;
DROP VIEW IF EXISTS `ctactesaldo`;
CREATE ALGORITHM=UNDEFINED DEFINER=`tulior`@`%` SQL SECURITY DEFINER VIEW `ctactesaldo` AS select `fac`.`entidad` AS `entidad`,(`fac`.`saldo` + `rec`.`saldo`) AS `saldo` from (`trsoftdb`.`ctactefact` `fac` left join `trsoftdb`.`ctacterec` `rec` on((`fac`.`entidad` = `rec`.`entidad`)));
...
DROP TABLE IF EXISTS `facturasaldo`;
DROP VIEW IF EXISTS `facturasaldo`;
CREATE ALGORITHM=UNDEFINED DEFINER=`tulior`@`%` SQL SECURITY DEFINER VIEW `facturasaldo` AS select `f`.`idfactura` AS `idfactura`,ifnull(sum(`c`.`imputado`),0) AS `cobrado`,(`f`.`total` - ifnull(sum(`c`.`imputado`),0)) AS `saldof` from (`trsoftdb`.`facturas` `f` left join `trsoftdb`.`cobros` `c` on((`f`.`idfactura` = `c`.`idfactura`))) where (not(`f`.`idfactura` in (select `ultimoestado`.`id` from `trsoftdb`.`ultimoestado` where ((`ultimoestado`.`tabla` = 'facturas') and (`ultimoestado`.`campo` = 'idfactura') and (`ultimoestado`.`idestador` = 2))))) group by `f`.`idfactura`;
