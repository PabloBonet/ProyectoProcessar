...
/*class ProgramaPrincipal
{
    ...
    public static int Main(string[] args)
    {
        ...
        {
            Console.WriteLine(ID_FNC + "***EXCEPCION AL CARGAR TICKET: " + excepcionAlCargarTicket.Message);
            return -10;
        }
        ...
        try
        {
            ...
            if (objTicketRespuesta == null)
            {
                ...
                //¿hay que guardar el Ticket en DEFAULT_TA?
            }
            else
            {
                //VER COMO OBTENER LA FECHA Y HORA SINCRONIZADA
                DateTime horaActual = new DateTime();
                horaActual = DateTime.Now;
                if ( objTicketRespuesta.ExpirationTime >= horaActual && objTicketRespuesta.GenerationTime <= horaActual )
                {
                    //No hago nada, se cargo el TA anteriormente
                }
                else
                {
                    //Se venció el TA, vuelvo a obtener otro
                    objTicketRespuesta = new LoginTicket();
                    if (blnVerboseMode) Console.WriteLine(ID_FNC + "***Accediendo a {0}", strUrlWsaaWsdl);
                    strTicketRespuesta = objTicketRespuesta.ObtenerLoginTicketResponse(strIdServicioNegocio, strUrlWsaaWsdl, strRutaCertSigner, strPasswordSecureString, strProxy, strProxyUser, strProxyPassword, blnVerboseMode);
           ...
           // Recuperar el ultimo valor del comprobante registrado
            

            try
            {
                ClienteLoginCms_CS.ar.gov.afip.wswhomo.Service servicio = new ClienteLoginCms_CS.ar.gov.afip.wswhomo.Service();
                ...
                autorizacion.Cuit = 30712443347;
                autorizacion.Sign = objTicketRespuesta.Sign;
                autorizacion.Token = objTicketRespuesta.Token;


                int ptoVenta = 1;
                int tipoComprobante = 1; //1 para facturas

                //LLamo al servicio FECompUltimoAutorizado para obtener el ultimo comprobante autorizado
                ClienteLoginCms_CS.ar.gov.afip.wswhomo.FERecuperaLastCbteResponse ultimoComprobante = null;

                ultimoComprobante = servicio.FECompUltimoAutorizado(autorizacion, ptoVenta, tipoComprobante);

                Console.WriteLine("\n\nUTLIMO COMPROBANTE AUTORIZADO\n\n");
                Console.WriteLine("\nNumero: {0}, Tipo: {1}, PtoVenta: {2}", ultimoComprobante.CbteNro, ultimoComprobante.CbteTipo, ultimoComprobante.PtoVta);


            }
            catch (Exception excepcionObtUltComprobante)
            {
                Console.WriteLine(ID_FNC + "***EXCEPCION AL CARGAR TICKET: " + excepcionObtUltComprobante.Message);
                return -10;
            }
           ...
           // string CAE = "";
            ...
            //ClienteLoginCms_CS.ar.gov.afip.wswhomo.DummyResponse respuesta = new ClienteLoginCms_CS.ar.gov.afip.wswhomo.DummyResponse();
            
           // CAE = ObtenerCAE(datosFactura, strTicketRespuesta);
    ...
    public static string ObtenerCAE(string datosFactura, string strTicketRespuesta)
    {
        string CAE = "";
